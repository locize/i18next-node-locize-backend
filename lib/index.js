  ...
  _createClass(Backend, [{
    ...
    value: function create(languages, namespace, key, fallbackValue, callback, options) {
      ...
      this.checkIfProjectExists(function (err) {
        ...
        // missing options
        var isMissing = utils.isMissingOption(_this7.options, ['projectId', 'version', 'apiKey', 'referenceLng']);
        if (isMissing) return callback(new Error(isMissing));
    ...
    value: function write(lng, namespace) {
      ...
      if (missings.length) {
        (function () {
          // lock
          utils.setPath(_this8.queuedWrites, ['locks', lng, namespace], true);

          var namespaceSaved = function namespaceSaved() {
            // unlock
            utils.setPath(_this8.queuedWrites, ['locks', lng, namespace], false);
          ...
          _this8.writePage(lng, namespace, page, function () {
            pagesDone++;
            if (pagesDone >= amountOfPages) namespaceSaved();
          });
          while (page.length === pageSize) {
            page = missings.splice(0, pageSize);
            if (page.length) _this8.writePage(lng, namespace, page, function () {
              pagesDone++;
              if (pagesDone >= amountOfPages) namespaceSaved();
            });
          }
